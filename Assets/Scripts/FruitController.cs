...
public class FruitController : MonoBehaviour
{
    ...
    void OnCollisionEnter2D(Collision2D collision) {
        if(collision.gameObject.tag == "Line")
        {
            ...
            c1.GetComponent<Rigidbody2D>().AddForce(new Vector2(2f, 2f), ForceMode2D.Impulse);      // add forec
            c1.GetComponent<Rigidbody2D>().AddTorque(Random.Range(-2f, 2f), ForceMode2D.Impulse);   // add random rotation

            c2.GetComponent<Rigidbody2D>().AddForce(new Vector2(-2f, 2f), ForceMode2D.Impulse);      // add forec
